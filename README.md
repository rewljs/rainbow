...
This result is generated by [`show.js`](./show.js).
...
### rainbow(content: string, options?: RainbowOptions)

Apply rainbow color to content using current style chaining.

[`bg`](#bg) modifier can be chained before this method to make the rainbow color applied to the background instead of the text.
...
### hash(content: string, options?: HashOptions)
...
[`bg`](#bg) modifier can be chained before this method to make the color applied to the background instead of the text.
...
The base renderer used in this module. Its implementation is [here](./src/impl/render.ts).
...
The list (an array of string) of all available colors.
